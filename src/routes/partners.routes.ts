...
// ── GET /partners/:slug ─────────────────────────────────
partnersRouter.get('/:slug', async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from('partners')
      .select(`
...
partnersRouter.post(
  '/apply',
  formLimiter,
  validate(partnerApplicationSchema),
  async (req, res, next) => {
    try {
...
// ── PATCH /partners/:id — admin update ─────────────────
...
// ── POST /partners/:id/sponsorships — admin register sponsorship ──
partnersRouter.post(
  '/:id/sponsorships',
  requireAuth,
  requireRole('admin'),
